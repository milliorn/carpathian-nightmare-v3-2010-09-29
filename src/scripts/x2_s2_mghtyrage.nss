...
void MightyWeapon(int nSlot, int nBonus,  int nRounds) {
   ...
   if (GetIsObjectValid(oWeapon)) {
      IPSafeAddItemProperty(oWeapon, ItemPropertyMassiveCritical(nBonus), RoundsToSeconds(nRounds), X2_IP_ADDPROP_POLICY_KEEP_EXISTING, TRUE, TRUE);
   }
}
...
void main() {
   if (!GetHasFeatEffect(FEAT_MIGHTY_RAGE)) {
      ...
      int nLevelMod   = 2 + nLevel/10;         // 4-6 (min level of Mighty Rage is 21)
      ...
      eLink = EffectLinkEffects(eLink, EffectSkillIncrease(SKILL_INTIMIDATE, nSkill));
