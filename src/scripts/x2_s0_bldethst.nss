...
// End of Spell Cast Hook
    ...
    if(GetIsObjectValid(oMyWeapon) && GetLevelByClass(CLASS_TYPE_RANGER, OBJECT_SELF) >=1)
    {
        ...
        if (GetSlashingWeapon(oMyWeapon))
        {
            if (nDuration>0)
            {
                ...
                IPSafeAddItemProperty(oMyWeapon, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_SLASHING,DAMAGE_BONUS_2d6), RoundsToSeconds(nDuration));
                IPSafeAddItemProperty(oMyWeapon, ItemPropertyKeen(), RoundsToSeconds(nDuration));
                IPSafeAddItemProperty(oMyWeapon, ItemPropertyEnhancementBonus(5), RoundsToSeconds(nDuration), X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE,TRUE);
                return;
            }
        }
    }
    else if(GetIsObjectValid(oMyWeapon) )
    {
        SignalEvent(oMyWeapon, EventSpellCastAt(OBJECT_SELF, GetSpellId(), FALSE));
