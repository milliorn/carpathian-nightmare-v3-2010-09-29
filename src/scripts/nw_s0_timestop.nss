...
const int DISTANCE_TO_CASTER = 30; // Meters
...
*/
const int MIN_DURATION = 6; // Seconds minumum
...
//  int iRoll = Random(DICE_ROLL) +1 + MIN_DURATION;
    int iRoll = 12;
    int iDC = GetSpellSaveDC() + GetCasterLevel(oCaster);
    ...
    if (GetLocalInt(oCaster, "TIME_STOP") == SPELL_TIME_STOP)
    {
        ...
        FloatingTextStringOnCreature("Timestop is only allowed once per turn(60 seconds)", oCaster);
        return;
    }

        SetLocalInt(oCaster, "TIME_STOP", SPELL_TIME_STOP);
        DelayCommand(60.0, DeleteLocalInt(oCaster, "TIME_STOP"));
    ...
    if (!GetLocalInt(GetArea(oCaster), "NO_PVP"))
    {
      ...
      while (GetIsObjectValid(oTarget))
        {
            if(oTarget != oCaster && !MySavingThrow(SAVING_THROW_REFLEX, oTarget, iDC - d20() , SAVING_THROW_TYPE_NONE, OBJECT_SELF, 0.0))
            {
