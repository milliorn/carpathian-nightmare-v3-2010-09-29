...
void ToggleEncounters(object oDM, int nActive)
{
    object oEncounter = GetFirstObjectInArea(oDM);
    while(oEncounter != OBJECT_INVALID)
    {
        if(GetObjectType(oEncounter) == OBJECT_TYPE_ENCOUNTER)
            SetEncounterActive(nActive, oEncounter);
        oEncounter = GetNextObjectInArea(oDM);
    }
    string sMsg;
    if(nActive == 0)
        sMsg = "DISABLED";
    ...
    SendMessageToPC(oDM, "All encounters in this area " +sMsg);
}
...
int GetIsChannelSuppressed(int nChannel)
{
    ...
    case 2: nReturn = GetLocalInt(GetModule(), "DISABLE_SHOUT_CHANNEL"); break; //DISABLE_SHOUT_CHANNEL;
