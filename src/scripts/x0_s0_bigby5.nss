...
void main()
{
    ...
    */
    ...
    int iExtraDC = GetHitDice(OBJECT_SELF) / 2;
    int iDC = GetSpellSaveDC() + iExtraDC;
    int iSave = GetSkillRank(SKILL_DISCIPLINE , oTarget);
    int iRoll = d20();
    //--------------------------------------------------------------------------
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        if(!MyResistSpell(OBJECT_SELF, oTarget))
        {
              ...
              if((iRoll + iSave) < iDC)
              {
                    DisplaySaveMessage(oTarget, iDC, iSave, iRoll, "Discipline");
