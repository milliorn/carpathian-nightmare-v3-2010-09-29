...
{
   IPSafeAddItemProperty(oTarget, ItemPropertyEnhancementBonus(6), fDuration, X2_IP_ADDPROP_POLICY_REPLACE_EXISTING,FALSE, TRUE);
   IPSafeAddItemProperty(oTarget, ItemPropertyOnHitProps(IP_CONST_ONHIT_DISPELMAGIC, IP_CONST_ONHIT_SAVEDC_26), fDuration,X2_IP_ADDPROP_POLICY_REPLACE_EXISTING );
    ...
    */
    ...
    if(GetIsObjectValid(oMyWeapon) )
    {
        ...
        if (GetBaseItemType(oMyWeapon) == BASE_ITEM_QUARTERSTAFF ||
            GetBaseItemType(oMyWeapon) == BASE_ITEM_SHORTSPEAR)
        {
            if (nDuration>0)
            {
